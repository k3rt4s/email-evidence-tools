...
| `scan_mbox_for_evidence.py`         | Scans an mbox file's Subject, From display names, Reply-To and message bodies for configurable evidence keyword categories. One row per hit, with a `location` column naming which (`subject`, `from_name`, `reply_to` or `body`).                                                  |
...
| `strip_attachments_from_mbox.py`    | Creates an attachment-free mbox copy and writes an attachment inventory CSV. A single-part message whose whole body is an attachment is kept, with a one-line placeholder naming the file, size and SHA-256 in place of its body.                                                   |
...
Every run that is not a dry run writes `custody_<UTC stamp>.json` in the case folder, a chain-of-custody record. It holds the SHA-256 of every source archive, streamed and taken before stage 1 (`--no-source-hash` skips this on archives too large to wait for), the code commit and whether the checkout was dirty, the exact command line, each stage's exit code and timing, and a size and SHA-256 listing of every file under the four stage folders, each tagged `new`, `modified`, `pre-existing`, `removed` or `unknown` against a snapshot taken before stage 1. A failed or interrupted run still gets a record, marked `status: incomplete` and naming the stage that stopped. If the record itself cannot be written the failure is logged, and a run that was otherwise clean exits 3 rather than 0.
...
Rendered exhibits include delivery evidence when the source message carries it. The message YAML block may include `return_path`, `auth_results`, `received_count`, numbered `received_N` entries, and numbered `dkim_signature_N` entries. `Received` headers are shown in the order they appear in the mbox, which is normally newest relay first because each relay prepends its own header. The message CSV appends `received_count`, `return_path`, and `auth_results` after its existing columns. The manifest adds `messages_with_received`, `messages_with_auth_results`, and `messages_with_dkim_signature` counts.
